...
public class CommissionSettingController {
    ...
    @ApiOperation("根据类型来查询")
    @RequestMapping(value = "/selectByCommissionCode/{commissionCode}", method = RequestMethod.GET, produces = {"application/json;charset=UTF-8"})
    ResponseMessage<CommissionSetting> selectByCommissionCode(@PathVariable("commissionCode") String commissionCode) {
        return new ResponseMessage<>(commissionSettingService.selectByCommissionCode(commissionCode));
    }

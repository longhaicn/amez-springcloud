...
public class BaseUmengPushTemplateServiceImpl implements BaseUmengPushTemplateService {
       ...
       public void push(List<BaseMemberDevice> list,BaseUmengPushTemplate baseUmengPushTemplate){
           if(!CollectionUtils.isEmpty(list)) {
               String token = list.get(0).getDeviceToken();
               if(token != null){
                   baseUmengPushTemplate.setDeviceToken(token);
                   Integer deviceType = list.get(0).getDeviceType();
                   //1为android
                   if(deviceType.equals(BaseUmengPushTemplate.DeviceType.ANDROID)){
                       baseUmengPushTemplate.setDeviceType(deviceType);
                       this.pushAndroidService.sendMessage(baseUmengPushTemplate);
                       //2为IOS
                   }else if(deviceType.equals(BaseUmengPushTemplate.DeviceType.IOS)){
                       baseUmengPushTemplate.setDeviceType(deviceType);
                       this.pushIosService.sendMessage(baseUmengPushTemplate);
                   }
               }
           }
       }

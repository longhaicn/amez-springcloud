...
public class CommissionSettingApiController {
    ...
    public ResponseMessage<Integer> updateByPrimaryKeySelectiveByList(@RequestBody List<CommissionSetting> record) {
        int resultCount = this.commissionSettingFeign.updateByPrimaryKeySelectiveByList(record);
        return new ResponseMessage<>(resultCount);
    }
    ...
    public ResponseMessage<PageInfo<CommissionSetting>> selectListByConditions(@ApiParam(value = "分页索引", defaultValue = "0") @RequestParam(defaultValue = "0") Integer pageNo, @ApiParam(value = "每页数量", defaultValue = "10") @RequestParam(defaultValue = "10") Integer pageSize, @RequestBody CommissionSetting record) {

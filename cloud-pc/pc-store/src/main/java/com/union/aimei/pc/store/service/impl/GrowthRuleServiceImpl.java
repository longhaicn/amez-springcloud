...
import com.union.aimei.pc.store.mapper.GrowthRuleLogMapper;
import com.union.aimei.pc.store.mapper.GrowthRuleMapper;
...
public class GrowthRuleServiceImpl implements GrowthRuleService {
    ...
    public ResponseMessage checkCode(byte ruleType, String code) {
        ...
        growthRule.setRuleType(ruleType);
        growthRule.setCode(code);
        List<GrowthRule> list = this.growthRuleMapper.selectListByConditions(growthRule);
        if (list.size() != 0) {
            responseMessage.setCode(GrowthRuleConstant.Save.GROWTH_RULE_CODE);
            responseMessage.setMessage(GrowthRuleConstant.Save.GROWTH_RULE_CODE_MESSAGE);
        }
    ...
    public ResponseMessage saveGrowthRule(GrowthRule growthRule) {
        ...
        //code校验
        responseMessage = checkCode(growthRule.getRuleType(), growthRule.getCode());
        if (ResponseUtil.isFail(responseMessage)) {
            return responseMessage;
        }
        //判断限制条件
        conditionSetType(growthRule);
        this.growthRuleMapper.insertSelective(growthRule);
        return responseMessage;
    }
    ...
    public ResponseMessage editGrowthRule(GrowthRule growthRule) {
        //id不可为空
        if (growthRule.getId() == null) {
            throw new ServerException(GrowthRuleConstant.Edit.LACK_ID, GrowthRuleConstant.Edit.LACK_ID_MESSAGE);
        }
        //判断限制条件
        conditionSetType(growthRule);
        this.growthRuleMapper.updateByPrimaryKeySelective(growthRule);
     ...
     * @param growthRule
     */
    public static void conditionSetType(GrowthRule growthRule) {
        if (growthRule.getConditionType() != null && GrowthRule.ConditionType.UNLIMITED == growthRule.getConditionType()) {
            //无限制
            growthRule.setConditionValueDay(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
            growthRule.setConditionValueMonth(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
            growthRule.setConditionValueNumber(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
        } else if (growthRule.getConditionType() != null && GrowthRule.ConditionType.TIMESLIMIT == growthRule.getConditionType()) {
            //次数限制
            growthRule.setConditionValueDay(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
            growthRule.setConditionValueMonth(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
        } else if (growthRule.getConditionType() != null && GrowthRule.ConditionType.NUMERICALLIMIT_DAY_MONTH == growthRule.getConditionType()) {
            //数值限制（日、月）
            growthRule.setConditionValueNumber(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
        } else if (growthRule.getConditionType() != null && GrowthRule.ConditionType.NUMERICALLIMIT_DAY == growthRule.getConditionType()) {
            //日限制
            growthRule.setConditionValueNumber(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
            growthRule.setConditionValueMonth(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
        } else if (growthRule.getConditionType() != null && GrowthRule.ConditionType.NUMERICALLIMIT_MONTH == growthRule.getConditionType()) {
            //月限制
            growthRule.setConditionValueNumber(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
            growthRule.setConditionValueDay(GrowthRuleConstant.CONDITION_TYPE_DEFAULT);
        }
        ...
        switch (growthRule.getRuleType()) {
            ...
            case GrowthRule.RuleType.BEAUTICIAN:
                if (BeauticianGrowthRuleEnum.SIGN_COURSE.getCode().equals(growthRule.getCode())) {
                    //课程
                    Course course = this.courseFeign.queryByIdV110(growthRuleVo.getObtainId());
